...
class SchNetVNModel(SchNet):
    """
    SchNet (short-range) + latent-charge Coulomb head (long-range).
    Fixed: Uses learnable scaling for VN to prevent oversmoothing.
    """
    def __init__(
        self,
        ...
        # Fix 1: Learnable scales for VN injection. 
        # Initialize to 0.0 to behave like standard SchNet initially.
        self.vn_scales = nn.Parameter(torch.zeros(num_layers))

        self.use_long_range = use_long_range
        if use_long_range:
            # Fix 2: Deeper Charge Head
            ...
            # Virtual Node MLP
            self.mlp_h_loc = nn.Sequential(
                nn.Linear(hidden_channels, hidden_channels),
                nn.SiLU(),
                nn.Linear(hidden_channels, hidden_channels),
            )

        self.reset_parameters_vn()

    def reset_parameters_vn(self):
        # Initialize VN scales to 0 to prevent initial feature collapse
        nn.init.zeros_(self.vn_scales)
        if hasattr(self, 'q_head'):
            for m in self.q_head:
                if isinstance(m, nn.Linear):
                    nn.init.xavier_uniform_(m.weight)
                    nn.init.zeros_(m.bias)
            # Ensure last layer of q_head starts small to avoid massive gradients
            nn.init.uniform_(self.q_head[-1].weight, -0.01, 0.01)
    ...
    def _coulomb_energy_dense(
        ...
        diff = pos_d.unsqueeze(2) - pos_d.unsqueeze(1) # (B, N, N, 3)
        r = (diff.pow(2).sum(dim=-1) + 1e-8).sqrt() # (B, N, N)
        ...
        U_coul = 0.5 * (pair * m).sum(dim=(1, 2)).view(-1, 1)
    ...
    def forward(self, batch, q_tot: Optional[torch.Tensor] = None, return_aux: bool = False):
        ...
        # Modified Interaction Loop
        for i, interaction in enumerate(self.interactions):
            # 1. Standard SchNet Interaction
            h_loc = self.activation(self.mlp_h(h) + interaction(h, edge_index, edge_weight, edge_attr))
            
            # 2. Virtual Node Mixing (with Scale)
            # Compute global context from local features
            h_vn_raw = self.mlp_h_loc(h_loc)
            h_global = scatter(h_vn_raw, batch.batch, dim=-2, dim_size=h_loc.size(0), reduce='mean')
            
            # Broadcast back and add with learnable scale
            # Initially scale is 0, so h = h_loc (pure SchNet)
            h = h_loc + h_global[batch.batch] * self.vn_scales[i]
        ...
        if not self.use_long_range:
            if return_aux:
                ...
                return U_sr, aux
            return U_sr, None # Return tuple for consistency

        # Fix 3: Compute Charges
