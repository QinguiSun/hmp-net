...
from hmp_layers.master_selection import MasterSelection
from hmp_layers.virtaul_generation import VirtualGeneration

Master_Module = MasterSelection()
Virtual_Module = VirtualGeneration()
...
class SchNetModel(SchNet):
    ...
    def forward(self, batch):
        ...
        for interaction in self.interactions:
            ...
            h_local = interaction(h, batch.edge_index, edge_weight, edge_attr)
            master_nodes = Master_Module.MasterSelection(h_local, batch.edge_index)
            virtual_edge = Virtual_Module.edge_generation(h_local, master_nodes)
            h_global = interaction(h_local, batch.edge_inbdex + virtual_edge, edge_attr)
            h = h + h_global
